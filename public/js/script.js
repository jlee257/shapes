...
$(document).ready(function () {
  ...
  function curveApproximation() {
    ...
    for (var i = 0, ilen = points.length; i < ilen; i++) {
      ...
      if (closest_j == 0 || closest_j == curve_points.length - 1) {
        e = distanceBetween(curve_points[closest_j].x, curve_points[closest_j].y, points[i].x, points[i].y);
        console.log("Curve: point=" + i + " curve=" + closest_j + " err=" + e);
      } else if (0 < closest_j && closest_j < curve_points.length - 1) {
        var dis_a = distanceFromLine(curve_points[closest_j-1].x, curve_points[closest_j-1].y, curve_points[closest_j].x, curve_points[closest_j].y, points[i].x, points[i].y);
        var dis_b = distanceFromLine(curve_points[closest_j].x, curve_points[closest_j].y, curve_points[closest_j+1].x, curve_points[closest_j+1].y, points[i].x, points[i].y);
        var dis_c = distanceBetween(curve_points[closest_j].x, curve_points[closest_j].y, points[i].x, points[i].y);
        e = Math.min(dis_a, dis_b, dis_c);
        console.log("Curve: point=" + i + " curve=" + closest_j + " dis_a=" + dis_a + " dis_b=" + dis_b + " dis_c=" + dis_c + " err=" + e);
      // } else if (closest_j == curve_points.length - 1){
      //   var dis_a = distanceFromLine(curve_points[closest_j-1].x, curve_points[closest_j-1].y, curve_points[closest_j].x, curve_points[closest_j].y, points[i].x, points[i].y);
      //   var dis_c = distanceBetween(curve_points[closest_j].x, curve_points[closest_j].y, points[i].x, points[i].y);
      //   e = Math.min(dis_a, dis_c);
      //   console.log("Curve: point=" + i + " curve=" + closest_j + " dis_a=" + dis_a + " dis_c=" + dis_c + " err=" + e);
      } else {
        e = 3000;
        console.log("Curve: point=" + i + " curve=" + closest_j + " err=" + e);
      }
